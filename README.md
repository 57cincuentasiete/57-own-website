...
The panel URL is hidden: visiting `/admin/` without a session takes you to
the homepage with the same sign-in popup as the **🔒 Admin** button. Sign in
there (or click the button) and you will be taken to the panel. Sign in with:
...
```powershell
.\setup-local-https.ps1   # one-time: creates a local HTTPS certificate in certs/
node worker/test-local.mjs --https
# then open https://127.0.0.1:8787/admin/
```

The local server supports plain HTTP (`node worker/test-local.mjs`) and HTTPS
(`--https`). The HTTPS mode uses the self-signed certificate generated by
`setup-local-https.ps1`, or your own PEM files via the `SSL_CRT_FILE` /
`SSL_KEY_FILE` environment variables. Because the certificate is self-signed,
your browser shows a one-time warning before continuing — that is expected
during local development.

The deployed site is always served over HTTPS by Cloudflare, and plain HTTP
requests to it are redirected to HTTPS automatically. To preview without TLS:
