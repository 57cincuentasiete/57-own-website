...
//
//   node worker/test-local.mjs            -> serves the site + admin panel on :8787 (HTTP)
//   node worker/test-local.mjs --https    -> same, over HTTPS (run setup-local-https.ps1 first)
...
// Returns Node https.createServer options for the local certificate, or null
// to serve plain HTTP. Preferred layout: certs/localhost.crt + localhost.key
// (e.g. from mkcert). Fallback: certs/localhost.pfx generated by
// setup-local-https.ps1, with its passphrase stored next to it.
function tlsOptions() {
  if (!USE_HTTPS || process.argv.includes("--selftest")) return null;

  const certFile = process.env.SSL_CRT_FILE || path.join(CERT_DIR, "localhost.crt");
  const keyFile = process.env.SSL_KEY_FILE || path.join(CERT_DIR, "localhost.key");
  ...
  if (fs.existsSync(pfxFile)) {
    const passFile = path.join(CERT_DIR, "localhost.pfx.pass");
    const passphrase = fs.existsSync(passFile)
      ? fs.readFileSync(passFile, "utf8").trim()
      : process.env.PFX_PASSPHRASE || "";
    return { pfx: fs.readFileSync(pfxFile), passphrase };
  }

  console.error("HTTPS requested but no local certificate was found.");
  console.error(`Looked for ${certFile} + ${keyFile}, or ${pfxFile}.`);
  console.error("Generate a certificate with:  .\\setup-local-https.ps1");
  console.error("(or point SSL_CRT_FILE / SSL_KEY_FILE at your own cert and key).");
  process.exit(1);
}
...
const server = (tls ? https : http).createServer(tls || {}, async (req, res) => {
  try {
    ...
    const request = new Request(`${scheme}://127.0.0.1${req.url}`, {
      method: req.method,
      headers,
      body,
    });
...
async function selfTest() {
  ...
  check(
    "guest /admin/ redirects to homepage sign-in modal",
    res.status === 302 && (res.headers.get("location") || "").includes("?admin=1"),
...
server.listen(PORT, "127.0.0.1", async () => {
  console.log(`Local CMS server: ${scheme}://127.0.0.1:${PORT}`);
  console.log(`Admin panel:       ${scheme}://127.0.0.1:${PORT}/admin/`);
